...
{
    public static void GenerateText(string mapname, NodeLayout layout, ElementManager mgr, List<NationData> nations, Vector2 mins, Vector2 maxs, List<ProvinceMarker> provs)
    {
        ...
        using (FileStream fs = File.Create(path))
        {
            ...
            write(fs, "#description \"Generated by Automated Improved Dominions Starts. Throne count: " + layout.NumThrones + "\"");

            write(fs, "\n-- Nation Data");

            foreach (NationData d in nations)
            {
                write(fs, "#allowedplayer " + d.ID);
            }
            ...
            foreach (ProvinceMarker m in provs)
            {
                if (m.Node.HasNation)
                {
                    write(fs, "#specstart " + m.Node.Nation.ID + " " + m.ProvinceNumber);
                }
            }
